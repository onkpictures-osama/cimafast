...
def _render_changes(project_id, company_id, cast, changes):
    ...
    if char.get("actor"):
        ...
        st.caption(f"🎬 {char['actor']['name']}"
                   + (f" — 📏 {' · '.join(bits)}" if bits else f" — {t('مقاسات الممثل مش متسجلة في بروفايله')}"))
    else:
        st.caption(t("لسه مفيش ممثل متعاقد للدور ده — المقاسات بتظهر هنا أول ما يتعاقد."))
