...
def render(page, current_user, project_id, project_company_id, home_company_id, role, is_ar):
    ...
    # في وضع "اختار لـ..." الشريط اللي فوق فيه رجوع للمكان/الشخصية نفسها - زرار
    # واحد بس بدل اتنين
    if not st.query_params.get("pick"):
    ...
    try:
        if page == "library":
            ...
            views.locations_library.render(project_id, project_company_id, st.query_params.get("pick"))
