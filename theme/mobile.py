...
الطبقات هنا متقسمة على مراحل الخطة:

  ‎TOUCH_CSS‎   (0) مساحات اللمس 44px — نفس الرقم المستخدم فعلاً لزرار
                    "إنشاء مشروع جديد" في الشريط الجانبي (‎theme/sidebar.py‎)،
                    مش رقم جديد.
  ‎NAV_CSS‎     (1) شريط التبويبات بيلفّ أسطر بدل ما يزحلق ويخبّي تبويبات،
                    وزرار الشريط الجانبي بقى في مقاس اللمس.
  ‎LAYOUT_CSS‎  (2) الصور والاختيارات والجداول.
  ‎TYPE_CSS‎    (3) مقاسات الخط والفراغات.

**كل رقم هنا اتقاس في متصفح على 390px قبل ما يتكتب.** الفحص اللي بيثبتهم
‎tests/visual/mobile_ui.py‎، والحد اللي بيحمي التابلت وسطح المكتب (كل حاجة
جوه ‎@media‎ واحدة) متغطي في ‎tests/test_theme.py‎.

سطر المراحل القديم (‎.cf-stepper‎) اتشال خالص من الشاشة في جولة تحسينات
سابقة (شوف ‎test_the_five_stage_cards_are_gone‎) واتبدل بـ ‎.cf-progress‎
اللي شكله كويس على الموبايل من غير أي تعديل، فمفيش داعي لستايل زيادة ليه.
...
NAV_CSS = f"""
    ...
    /* مقبض تكبير الشريط الجانبي بتاع Streamlit — شريط عرضه 8px واقف على حرف
       الشريط، 6 منهم بره الحرف. اتقاس في المرحلة 4: هو السبب الوحيد لزحلقة
       أفقية جوه الشريط في الإنجليزي (‎scrollWidth - clientWidth = 6px‎؛ في
       العربي بيقع على الحرف التاني فمبيعملهاش). وعلى التليفون الشريط أصلًا
       طبقة بتغطي الشاشة، ومقبض 8px مستحيل يتمسك بالصباع (خُمس الحد الأدنى
       44px) — فبيتشال تحت نقطة الكسر بس. فوق 768px المقبض زي ما هو.

       الحاوية اللي شايلاه لازم تتشال هي كمان: إخفاء المقبض لوحده سابها 8px
       واقفة في نفس المكان والزحلقة زي ما هي (اتقاس: 6px برضه). ‎:has(> …)‎
       بتمسك الحاوية من غير ما نعتمد على كلاس Streamlit المتولّد. */
    [data-testid="stSidebarResizeHandle"],
    div:has(> [data-testid="stSidebarResizeHandle"]) {{
